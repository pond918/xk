              ...
              success: (res) => {
                ...
                if (res.data.errorCode === 200) {
                  ...
                  if (data.role > 0) {
                    ...
                    // 如果用户没有注册，则去判断当前页面的role值
                    // 当当前页面的role为-1时，表示是未登陆未注册的老师通过二维码扫描进来注册页
                    // 这时，就不需要提示用户去注册，因为已经是在注册页了

                    // 默认角色以页面当前的角色为主。如果当前页的role不存在，则默认为是老师扫码进来的
                    let role = currentPage.role;

                    // 表示是未注册用户进入其他页面，而不是进入注册页，要提示跳转注册，走reject流程
                    if(role != -1 ){
                      return reject({
                        errorCode: 403,
                      ...
                      // role == -1 表示是未注册用户进入注册页，不用提示跳转注册，但是还要走reject流程
                      return reject({
                        errorCode: 200,
                        moreInfo: ''
                      });
